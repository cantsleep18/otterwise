...
function otterwiseApiPlugin(): Plugin {
  const primaryDir = path.resolve(__dirname, '../../.otterwise');
  const demoDir = path.resolve(__dirname, '../demo/.otterwise');
  const otterwiseDir = fs.existsSync(primaryDir) ? primaryDir : demoDir;
